...
function TestScene:initReferences()
	...
	self.defaultLayer:setCamera ( camera )
...
function TestScene:createShaderProgram()
...
function TestScene:createShader()
	local shader = MOAIShader.new ()
	shader:setProgram ( self.program )
	self.shader = shader
end

function TestScene:loadMesh()
	...
	prop:setDeck ( mesh )
	-- prop:setLoc( -50, 20, -128 )
	-- prop:setRot( 0, 30, 0 )
	-- prop:seekRot ( 180, 360, 0, 10 )
	prop:setCullMode ( MOAIGraphicsProp.CULL_BACK )
	self.defaultLayer:insertProp ( prop )
end

function TestScene:addMyMesh()
	-- default
	local prop = MOAIProp.new()
	...
	prop:setLoc( 0, 0, 0 )
	-- prop:setRot( 30, 30, 30 )
	-- prop:seekRot ( 180, 360, 0, 10 )

	self.defaultLayer:insertProp( prop )
end

function TestScene:createMesh()
	...
	addControlPoint( 3, 0.5, 10 ) -- 0 +
	addControlPoint( 3, 0.5, -10 ) -- 1 +
	addControlPoint( 3, -0.5, 10 ) -- 2 +
	addControlPoint( 3, -0.5, -10 ) -- 3 +
	addControlPoint( -3, 0.5, -10 ) -- 4 +
	addControlPoint( -3, 0.5, 10 ) -- 5 +
	addControlPoint( -3, -0.5, -10 ) -- 6 +
	addControlPoint( -3, -0.5, 10 ) -- 7 +
